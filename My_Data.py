...
tabela_movimentacoes= navegador.find_element("id",'table')
...
while True:
    for tr in tabela_movimentacoes.find_elements("tag name", "tr"):
        for td in tabela_movimentacoes.find_elements("tag name", "td"):
            for p in tabela_movimentacoes.find_elements("tag name", "p"):
            ...
            try:
                ...
                break
df_my_data = pd.DataFrame(my_data, columns=['image','corp','site','type','country','city'])
print(my_data)
time.sleep(50)
navegador.quit()
